...
func _update_character_density() -> void:
	...
	_header_subtitle.visible = not compact and not _last_sheet_route in ["attack", "defence", "cast", "use-item"]
...
func _on_sheet_workflow_changed(route: String, title: String, can_submit: bool, busy: bool) -> void:
	...
	_header_title.theme_type_variation = "RookframeTitle" if route in ["attack", "defence", "cast", "use-item"] else "RookframeHeading"
	if route in ["attack", "defence", "cast", "use-item"]:
		_header_title.text = title.to_upper()
	elif _character_actor != null:
		var data: Dictionary = _character_actor.data
		_header_title.text = str(data.get("name", "Unnamed Character")).to_upper()
	_header_subtitle.visible = not _compact and not route in ["attack", "defence", "cast", "use-item"]
	...
	get_node(^"Layout/SheetActions").visible = route in ["omens", "attack", "defence", "cast", "use-item"]
	get_node(^"Layout/SheetActions/Spend").visible = route == "omens"
	get_node(^"Layout/SheetActions/Attack").visible = route in ["attack", "defence", "cast", "use-item"]
	...
	get_node(^"Layout/SheetActions/Back").text = "Back to sheet" if route in ["attack", "defence", "cast", "use-item"] and not can_submit and not busy else "Cancel"
	...
	get_node(^"Layout/SheetActions/Back").disabled = busy and not route in ["attack", "defence", "cast", "use-item"]
