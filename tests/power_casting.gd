...
func test_palms_uses_only_equipped_intact_protection_and_preserves_zero_hp_boundary() -> void:
	for condition in ["shield", "broken", "unequipped", "empty"]:
		var host := _host("palms-open-the-southern-gate")
...
func test_immediate_target_count_and_hp_data_are_validated_before_requesting_dice() -> void:
	for invalid in ["too-many", "hp", "maximum", "dead", "no-maximum"]:
		var host := _host("grace-of-a-dead-saint")
		host.actors.enemy.data["maximum_hit_points"] = 12
		if invalid == "too-many":
			for index in 3:
...
func test_immediate_corrections_interruption_and_atomic_refusal() -> void:
	for failure in ["count", "duplicate", "range", "cancel", "disconnect", "owner", "relink", "malformed", "save"]:
		var host := _host("roskoes-consuming-glare")
		var sdk := SDK.new(host)
		await sdk.system_actions.submit("power.start", _cast_input())
		host.roll("cast", [11])
...
func test_shared_and_long_public_labels_keep_per_target_dice_distinct() -> void:
	for label in ["Hooded stranger", "𐀀".repeat(80)]:
		var host := _host("roskoes-consuming-glare")
